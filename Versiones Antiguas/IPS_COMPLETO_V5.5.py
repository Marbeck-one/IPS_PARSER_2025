...
ARCHIVOS = {
    ...
    "MAPA_POND": "FCM_INFO_2025_v3_RMB(INDICADORES_PONDERADOS).csv"
}
...
def limpiar_codigo_indicador_fase5(raw_num, contador, etiqueta):
    ...
    if es_invalido or es_nuevo:
        return f"INDICADOR_NUEVO_{contador}_{etiqueta}", True
    else:
        return cod_str, False

# --- NUEVA FUNCIÓN DE NORMALIZACIÓN ROBUSTA (PARA SOLUCIONAR AT/AV) ---
def normalizar_clave_responsable(texto):
    """
    Normaliza el nombre del departamento para cruzar Excel con CSV.
    Quita tildes, mayúsculas, espacios extra y el prefijo CDC.
    Ej: "CDC División Juridica" -> "DIVISION JURIDICA"
    Ej: "División Jurídica" -> "DIVISION JURIDICA"
    """
    ...
    # 2. Quitar prefijo CDC si existe
    txt = txt.replace("CDC ", "")
    
    # 3. Eliminar Tildes (NFD form separa caracteres de sus acentos)
    txt = ''.join(c for c in unicodedata.normalize('NFD', txt) if unicodedata.category(c) != 'Mn')
    
    # 4. Eliminar caracteres raros (puntos, comas)
...
def cargar_mapa_ponderados():
    if not os.path.exists(ARCHIVOS["MAPA_POND"]):
        print(f"[AVISO] No se encontró {ARCHIVOS['MAPA_POND']}. Columna AT (COD_PONDERADO) quedará vacía.")
        return {}
    try:
        df_map = pd.read_csv(ARCHIVOS["MAPA_POND"], sep=";")
        # NORMALIZACIÓN ROBUSTA DE LA CLAVE DEL CSV
        mapa = {}
        for _, row in df_map.iterrows():
            nombre_raw = row.get('NOMBRE', '')
            clave = normalizar_clave_responsable(nombre_raw)
            valor = row.get('INDICADOR_COD', '')
            if clave: mapa[clave] = valor
        return mapa
    except Exception as e:
        print(f"[ERROR] Leyendo CSV mapa ponderados: {e}")
        return {}

def transformar_a_indicadores_aplicados(df_origen, etiqueta_origen, contador_nuevos_global, mapa_ponderados):
    filas_app = []
    ...
    for index, row in df_origen.iterrows():
        ...
        # --- AT. COD_PONDERADO (MATCHING) ---
        responsable_raw = row.get('RESPONSABLE CENTRO DE RESPONSABILIDAD', '')
        # Aplicamos la misma normalización "agresiva" que al CSV
        responsable_norm = normalizar_clave_responsable(responsable_raw)
        
        cod_ponderado = mapa_ponderados.get(responsable_norm, None)
        ...
        fila = {
            ...
            'JER_TIPO_COD': 1,
            'CENTRO_RESP_COD': None, # Nota: Futuro uso
            'COD_REGION': None,      # Nota: Futuro uso
            'EMAIL_RESPONSABLE': 'prueba@arbol-logika.com',
            'COD_GENERO': None,      # Nota: Futuro uso
            'ANO_MES_INI': 202501, 'ANO_MES_FIN': 202512,
            ...
            'COMP_A': f"{codigo_indicador}_A",
            'COMP_A_CR': None, 'COMP_A_GEN': None, 'COMP_A_REGION': None,
            'COMP_B': f"{codigo_indicador}_B",
            'COMP_B_CR': None, 'COMP_B_GEN': None, 'COMP_B_REGION': None,
            'CONST_A': None,
            'META_202512': meta_val,
            'Ponderacion': ponderacion_val,
            'COD_PONDERADO': cod_ponderado,
            'FORMULA_VAR_AUTO': 'SUMA_ANUAL',
            'COD_VAR_AUTO': cod_var_auto
        }
...
def ejecutar_fase_5(dataframes_input=None):
    ...
    mapa_pond = cargar_mapa_ponderados()
    dfs_consolidados = []; contador_ind_global = 1
    
    # Columnas ordenadas
    cols_orden = [
        'INDICADOR_COD', 'NOMBRE_INDICADOR', 'JER_TIPO_COD', 'CENTRO_RESP_COD', 'COD_REGION',
        'EMAIL_RESPONSABLE', 'COD_GENERO', 'ANO_MES_INI', 'ANO_MES_FIN', 'COD_ANALISIS_CAUSA',
        'EMAIL_RESP_ANALISIS_CAUSA', 'CREAR_COMENTARIO_FORM',
        'ENE', 'FEB', 'MAR', 'ABR', 'MAY', 'JUN', 'JUL', 'AGO', 'SEP', 'OCT', 'NOV', 'DIC',
        'ORIGEN', 'NOTAS_SUPUESTOS', 'MOSTRAR_PANEL', 'APLICA_RIESGO',
        'OBJ_ESTRATEGICO_COD', 'OBJ_ESPECIFICO_COD', 'PROD_ESTRATEGICO_COD',
        'COD_PROGRAMA', 'COD_COMPONENTE_PROG', 'TIPO_META_ANUAL',
        'COMP_A', 'COMP_A_CR', 'COMP_A_GEN', 'COMP_A_REGION',
        'COMP_B', 'COMP_B_CR', 'COMP_B_GEN', 'COMP_B_REGION',
        'CONST_A', 'META_202512', 'Ponderacion', 'COD_PONDERADO',
        'FORMULA_VAR_AUTO', 'COD_VAR_AUTO'
    ]

    for etiqueta in ["CDC", "Riesgos", "PMG"]:
        if etiqueta in dataframes_input:
            df_app, nuevo_cont = transformar_a_indicadores_aplicados(dataframes_input[etiqueta], etiqueta, contador_ind_global, mapa_pond)
            contador_ind_global = nuevo_cont
            if not df_app.empty:
                fila_titulo = {col: None for col in cols_orden}; fila_titulo['INDICADOR_COD'] = f"--- {etiqueta.upper()} APLICADOS ---"
                dfs_consolidados.append(pd.DataFrame([fila_titulo]))
                dfs_consolidados.append(df_app)
...
def aplicar_estilos_planillas(n): pass # Simplificado
...
def menu_principal():
    while True:
        print("\n" + "="*60)
        print("   SISTEMA INTEGRAL IPS 2026 (FASES 1-5)")
        print("="*60)
        ...
        print("   6. Fase 5: Indicadores Aplicados (FIXED)")
        print("   0. Salir")
