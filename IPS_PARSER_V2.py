...
# ==========================================
# 1. MOTOR DE PROCESAMIENTO UNIVERSAL
# ==========================================
...
def procesar_hoja_universal(ruta_archivo, nombre_hoja_excel, prefijo_salida, opciones_salida):
    """
    Procesa una hoja especifica del Excel maestro.
    """
    print(f"\n--- Iniciando proceso para: {prefijo_salida} ---")
    print(f"1. Leyendo archivo: {ruta_archivo} (Hoja: {nombre_hoja_excel})...")
    
    try:
        # LEEMOS SIEMPRE COMO EXCEL AHORA
        df_raw = pd.read_excel(ruta_archivo, sheet_name=nombre_hoja_excel, header=None)
    except ValueError as e:
        print(f"ERROR: La hoja '{nombre_hoja_excel}' no existe en el archivo. Verifica el nombre exacto.")
        return
    ...
    for idx in indices_inicio:
        ...
        # 4. Operandos (Lógica de Intervalos +3, +5)
        if idx_op_desc:
...
# ==========================================
# 2. MENÚ PRINCIPAL CORREGIDO
# ==========================================

def menu_principal():
    print("="*60)
    print("   SISTEMA DE EXTRACCIÓN DE INDICADORES 2025")
    print("   (Un Archivo Excel -> Múltiples Hojas)")
    print("="*60)
    
    # 1. ARCHIVO MAESTRO
    # Asegúrate que este sea el nombre correcto del archivo que tienes en la carpeta
    archivo_maestro = "Proyecciones Indicadores 2025 - División Planificación (1).xlsx"
    
    if not os.path.exists(archivo_maestro):
        print(f"ERROR: No se encuentra el archivo maestro '{archivo_maestro}' en la carpeta actual.")
        return

    # ---------------------------------------------------------
    # 1. PROCESAR CDC (Hoja: CDC 2025)
    # ---------------------------------------------------------
    resp = input(f"\n¿Procesar Hoja 'CDC 2025'? (s/n): ").lower().strip()
    if resp == 's':
        print("  1. Solo Bruta\n  2. Solo Estilizada\n  3. Ambas")
        op = input("  Elige (1-3): ").strip()
        if op in ["1","2","3"]: 
            procesar_hoja_universal(archivo_maestro, "CDC 2025", "CDC", op)

    # ---------------------------------------------------------
    # 2. PROCESAR RIESGOS (Hoja: Riesgos 2025)
    # ---------------------------------------------------------
    resp = input(f"\n¿Procesar Hoja 'Riesgos 2025'? (s/n): ").lower().strip()
    if resp == 's':
        print("  1. Solo Bruta\n  2. Solo Estilizada\n  3. Ambas")
        op = input("  Elige (1-3): ").strip()
        if op in ["1","2","3"]: 
            # Le pasamos EL MISMO archivo maestro, pero cambiamos el nombre de la hoja
            procesar_hoja_universal(archivo_maestro, "Riesgos 2025", "Riesgos", op)

    # ---------------------------------------------------------
    # 3. PROCESAR PMG (Hoja: PMG 2025)
    # ---------------------------------------------------------
    resp = input(f"\n¿Procesar Hoja 'PMG 2025'? (s/n): ").lower().strip()
    if resp == 's':
        print("  1. Solo Bruta\n  2. Solo Estilizada\n  3. Ambas")
        op = input("  Elige (1-3): ").strip()
        if op in ["1","2","3"]: 
            # Le pasamos EL MISMO archivo maestro, pero cambiamos el nombre de la hoja
            procesar_hoja_universal(archivo_maestro, "PMG 2025", "PMG", op)
