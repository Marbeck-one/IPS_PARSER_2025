...
# =============================================================================
# IPS_HYBRID_v1.1.2 - DATA PRESENCE CHECK (GHOST ROW KILLER)
# =============================================================================

class IPSParserHybridV112:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v1.1.2_OCT-NOV-DIC_2025.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v1.1.2")
        print("="*60)
    ...
    def has_data_values(self, df, row_idx, c_map):
        """Verifica si la fila tiene al menos un dato numérico en columnas clave."""
        keys_to_check = ["meta", "Oct.", "Nov.", "Dic."]
        for key in keys_to_check:
            col_idx = c_map.get(key)
            ...
            for sheet in xls.sheet_names:
                ...
                for loop_idx, h_idx in enumerate(header_indices):
                    ...
                    for i in range(h_idx + 1, end_idx):
                        ...
                        # --- NUEVO: CHECK DE DATOS PARA EVITAR FILAS FANTASMA ---
                        # Si la fila no tiene número, verificar si tiene datos numéricos antes de preguntar
                        if not raw_num or raw_num.lower() == "nan":
                            has_data = self.has_data_values(df, i, c_map)
                            if not has_data: continue # Si no tiene datos, saltar silenciosamente (es nota o basura)
                        ...
                        rows_to_ignore = set(ignored_rows); rows_to_ignore.add(idx_ind_data)
...
if __name__ == "__main__":
    ...
    IPSParserHybridV112(path).process_folder()