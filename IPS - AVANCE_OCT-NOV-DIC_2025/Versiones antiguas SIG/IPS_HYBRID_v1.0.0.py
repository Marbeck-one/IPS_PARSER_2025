...
# =============================================================================
# IPS_PARSER_HYBRID_v7 - MOTOR v5 + SEGURIDAD v4
# =============================================================================

class IPSParserHybrid:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v7_HYBRID.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
        ...
        # Memorias de decisión (Heredadas de v4.0.2 + v5)
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v7 (MOTOR v5 + ERRORES v4)")
        print("="*60)
    ...
    # --- MÓDULOS DE INTERACCIÓN (IMPORTADOS DE v4.0.2) ---
    
    def ask_user_decision(self, key, prompt_text):
        """Pregunta simple Si/No con memoria."""
        ...
        while True:
            ...
            if resp in ["S", "N"]:
                decision = (resp == "S")
                # Solo memorizar si es una decisión estructural global
    ...
    def ask_column_action(self, missing_cols, sheet_name):
        print(f"\n[ALERTA] En hoja '{sheet_name}' faltan columnas críticas: {missing_cols}")
        print("   Opciones:")
        print("   [c] Continuar (Rellenar con 'No aplica' o intentar inferir).")
        print("   [s] Saltar esta hoja.")
        print("   [f] Saltar este ARCHIVO completo.")
        print("   [d] Detener y Guardar lo que llevamos.")
        while True:
    ...
    def ask_weird_row_action(self, row_idx, content, sheet_name):
        ...
        # Filtros automáticos de basura (Mejora v5)
    ...
    def get_hidden_rows(self, file_path, sheet_name):
        try:
            ...
            for row_idx, row_dim in ws.row_dimensions.items():
                if row_dim.hidden:
                    hidden.add(row_idx - 1) # Pandas usa 0-index
            wb.close()
            return hidden
        except: return set()
        ...
        if not candidate:
            ...
            for r_idx, c_idx in targets:
                if r_idx < len(df) and c_idx < len(df.columns):
                    ...
                    val_up = val.upper()
                    if (len(val) > 3 and val_up != "NAN" and "INDICADOR" not in val_up and "FECHA" not in val_up and "ELABORADO" not in val_up and "NUMERO" not in val_up):
                        candidate = val
                        break

        if not candidate or candidate.upper() in ["NO APLICA", "NAN"]:
            # Fallback interactivo (estilo v4) si falla el auto-detect
            print(f"\n{'!'*60}")
            print(f"[DECISIÓN MANUAL] No detecté el EQUIPO en: {file_name}")
            print(f"Hoja: {sheet_name}."); print(f"{'!'*60}")
            print("   [m] Escribir nombre MANUALMENTE.")
            print("   [n] Asignar 'No aplica'.")
            while True:
                choice = input("   >> Elija (m/n): ").lower().strip()
                if choice == 'm': 
                    candidate = input("   >> Ingrese Nombre del Equipo: ").strip()
                    break
                if choice == 'n': 
                    candidate = "No aplica"
                    break
    ...
    def find_operand_offsets_precise(self, df, start_row, ignored_rows):
        offset_op1 = None; offset_op2 = None; max_look = 12
        for offset in range(1, max_look + 1):
            ...
            if "OPERANDO 1=" in row_str or "OPERANDO 1 =" in row_str: offset_op1 = offset + 1 
            if "OPERANDO 2=" in row_str or "OPERANDO 2 =" in row_str: offset_op2 = offset + 1 
            if offset_op1 and offset_op2: break
        
        if not offset_op1 and not offset_op2:
            return self.find_operand_offsets_legacy(df, start_row, ignored_rows)
        return offset_op1, offset_op2

    def find_operand_offsets_legacy(self, df, start_row, ignored_rows):
        offsets = []
        for offset in range(1, 10):
            target = start_row + offset
            if target >= len(df) or target in ignored_rows: continue
            row_str = "".join([str(x).upper() for x in df.iloc[target].values])
            if any(c.isdigit() for c in row_str) and "VALOR" not in row_str: offsets.append(offset)
    ...
    def process_folder(self):
        ...
        for idx_file, file_path in enumerate(valid_files):
            ...
            # GESTIÓN DE OCULTOS (v4 style)
            try: xls = pd.ExcelFile(file_path)
            except Exception as e: print(f" [ERROR] Archivo corrupto: {e}"); continue

            for sheet in xls.sheet_names:
                # Determinar filas a ignorar
                hidden_rows = self.get_hidden_rows(file_path, sheet)
                ignored_rows = set()
                if hidden_rows:
                    if self.opt_hidden_strategy == 'visible': ignored_rows = hidden_rows
                    elif self.opt_hidden_strategy == 'all': ignored_rows = set()
                    else:
                ...
                if global_center is None: continue # Usuario eligió saltar en la pregunta manual

                # Detectar encabezados (Bloques)
                header_indices = []
                ...
                if not header_indices:
                    # Intento de rescate si no hay encabezados claros
                    # En v4 se preguntaba, aquí asumimos skip para no colgar, o podríamos preguntar.
                    pass
                ...
                for loop_idx, h_idx in enumerate(header_indices):
                    ...
                    if col_num is None:
                        # MANEJO DE ERROR V4: PREGUNTAR
                        action = self.ask_column_action("NÚMERO", sheet)
                        if action == 'skip_sheet': break
                        if action == 'skip_file': break # Necesitaría flag externo
                        if action == 'continue': 
                            if self.ask_user_decision("use_col_a_as_num", "¿Intentar usar Columna A como NÚMERO?"): col_num = 0
                            else: col_num = None # Se manejará en el loop de filas
                    ...
                    for i in range(h_idx + 1, end_idx):
                        ...
                        # --- FILTRO Y MANEJO DE FILAS RARAS (V4) ---
                        is_blacklisted = raw_num.upper() in self.blacklist_auto
                        is_value_row = "VALOR" in raw_num.upper()
                        is_valid_format = re.match(r'^\d', raw_num) or "NUEVO" in raw_num.upper()
                        
                        if not raw_num: continue # Skip vacíos silenciosamente
                        if is_blacklisted or is_value_row: continue

                        if not is_valid_format:
                            # Aquí es donde v4 preguntaba
                            action = self.ask_weird_row_action(i+1, raw_num, sheet)
                            if action == 'skip': continue
                            if action == 'auto':
                                # Generar un ID temporal para que no rompa
                        ...
                        off1, off2 = self.find_operand_offsets_precise(df, i, ignored_rows)
...
if __name__ == "__main__":
    ...
    IPSParserHybrid(path).process_folder()