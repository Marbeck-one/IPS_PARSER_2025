...
# =============================================================================
# IPS_HYBRID_v1.0.8 - COLUMN ZERO CHECK (LOS RIOS FIX)
# =============================================================================

class IPSParserHybridV108:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v1.0.8.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
        ...
        self.file_teams = {} 
        # Memorias por archivo
        self.file_auto = {}

    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v1.0.8")
        print("="*60)
    ...
    def process_folder(self):
        ...
        for idx_file, file_path in enumerate(valid_files):
            ...
            self.file_auto = {} # Reiniciar automatización por archivo
            ...
            for sheet in xls.sheet_names:
                ...
                for loop_idx, h_idx in enumerate(header_indices):
                    ...
                    for i in range(h_idx + 1, end_idx):
                        ...
                        # 2. EXTRACCIÓN INTELIGENTE (COL A O TEXTO)
                        if not raw_num or raw_num.lower() == "nan":
                            # A. Probar Columna A si no es la columna num
                            if c_map["num"] is None:
                                col0_val = str(df.iloc[i, 0]).strip()
                                # Busca X.X.X al inicio
                                if re.match(r'^\d+(\.\d+)+', col0_val):
                                    raw_num = col0_val # Silenciosamente aceptado si parece ID
                            
                            # B. Si falla, probar Texto Indicador
                            if not raw_num or raw_num.lower() == "nan":
                            ...
                            # C. Si falla todo, preguntar
                            if not raw_num or raw_num.lower() == "nan":
                                ind_content = str(df.iloc[i, c_map["ind"]]).strip() if c_map["ind"] is not None else ""
                                # Solo preguntar si parece una fila con datos (tiene texto largo en indicador)
                                if len(ind_content) > 5:
                                ...
                                else: continue # Si no tiene ni numero ni texto, saltar
...
if __name__ == "__main__":
    ...
    IPSParserHybridV108(path).process_folder()