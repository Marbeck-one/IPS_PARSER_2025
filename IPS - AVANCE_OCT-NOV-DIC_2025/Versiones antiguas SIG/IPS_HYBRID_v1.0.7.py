...
# =============================================================================
# IPS_HYBRID_v1.0.7 - AUTOMATION & PERSISTENCE
# =============================================================================

class IPSParserHybridV107:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v1.0.7.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
        ...
        # Memorias por Archivo (Se reinician en cada process_file)
        self.file_auto = {} 
        self.file_teams = {} 

    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v1.0.7 (AUTOMATIZADA)")
        print("="*60)
    ...
    def ask_segment_confirmation(self, text, context):
        # 1. Chequear si ya automatizamos esto para este archivo
        if self.file_auto.get("segment_always_yes"):
            return True
    ...
    def ask_id_extraction(self, extracted_id, context):
        if self.global_decisions.get("use_embedded_id") is not None:
            return self.global_decisions["use_embedded_id"]
        
        print(f"\n[DECISIÓN DE ID] {context}")
        print(f"   No hay columna Número, pero encontré '{extracted_id}' dentro del texto.")
        print("   [s]  Usar este código.")
        print("   [a]  Usar SIEMPRE códigos encontrados en texto (Global).")
        print("   [n]  No usar.")
        while True:
            choice = input("   >> Elija: ").lower().strip()
            if choice == 's': return True
            if choice == 'a': 
                self.global_decisions["use_embedded_id"] = True
                return True
            if choice == 'n': return False

    def ask_missing_id_strategy(self, prev_id, context):
        # 1. Chequear automatización por archivo
        if self.file_auto.get("missing_id_strat"):
            return self.file_auto["missing_id_strat"]
        ...
        while True:
            ...
            if choice == 'sa': 
                self.file_auto["missing_id_strat"] = 'skip'
                return 'skip'
            if choice == 'p': return 'prev'
            if choice == 'pa':
                self.file_auto["missing_id_strat"] = 'prev'
                return 'prev'
            if choice == 'n': return 'new'

    def ask_column_action(self, missing_cols, context):
        # Chequear si ya dijimos continuar siempre en este archivo
        if self.file_auto.get("missing_col_continue"):
            return 'continue'
        ...
        while True:
            ...
            if choice == 'ca': 
                self.file_auto["missing_col_continue"] = True
                return 'continue'
    ...
    def process_folder(self):
        ...
        for idx_file, file_path in enumerate(valid_files):
            ...
            # REINICIAR AUTOMATIZACIÓN POR ARCHIVO
            self.file_auto = {} 
                    ...
                    if h_idx > 0:
                        ...
                        if len(prev) == 1:
                            ...
                            if len(cand) < 60:
                                ...
                                if cand.upper() in self.known_segments: 
                        ...
                        if c_map["num"] is None or c_map["num"] == 0:
                            ...
                            if len(possible_seg) > 2 and len(possible_seg) < 30 and not any(c.isdigit() for c in possible_seg) and "INDICADOR" not in possible_seg.upper() and possible_seg != "" and possible_seg.upper() != "NAN":
                                ...
                                if self.ask_segment_confirmation(possible_seg, ctx):
                                    current_segment = possible_seg
                                    self.known_segments.add(possible_seg.upper())
                                    continue
...
if __name__ == "__main__":
    ...
    IPSParserHybridV107(path).process_folder()