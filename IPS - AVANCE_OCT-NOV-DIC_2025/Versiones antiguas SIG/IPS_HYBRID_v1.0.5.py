...
# =============================================================================
# IPS_HYBRID_v1.0.5 - CONTEXT AWARE & SMART SKIP
# =============================================================================

class IPSParserHybridV105:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v1.0.5.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v1.0.5")
        print("="*60)
    ...
    def get_context_str(self, filename, sheet, row):
        return f"[{filename}] > [{sheet}] > Fila {row}"

    def ask_user_decision(self, key, prompt_text, context=""):
    ...
    def ask_id_extraction(self, extracted_id, context):
        ...
        print(f"   No hay columna Número, pero encontré '{extracted_id}' en el texto.")
                    ...
                    if h_idx > 0:
                        ...
                        if len(prev) == 1:
                            ...
                            if len(cand) < 60:
                                ...
                                msg = f"Título '{cand}'. ¿Es SEGMENTO?"
                                if self.ask_user_decision("use_segment", msg, ctx): 
                    ...
                    for i in range(h_idx + 1, end_idx):
                        ...
                        if c_map["num"] is None or c_map["num"] == 0:
                            ...
                            if len(possible_seg) > 2 and len(possible_seg) < 30 and not any(c.isdigit() for c in possible_seg) and "INDICADOR" not in possible_seg.upper() and possible_seg != "" and possible_seg.upper() != "NAN":
                                msg = f"Texto '{possible_seg}'. ¿Es SEGMENTO?"
                                if self.ask_user_decision("use_segment_inline", msg, ctx):
                        ...
                        if not raw_num or raw_num.lower() == "nan":
                            # PRE-FILTRO: ¿Está la fila vacía de contenido real?
                            ind_content = str(df.iloc[i, c_map["ind"]]).strip() if c_map["ind"] is not None else ""
                            if len(ind_content) < 5: 
                                # Si no tiene número Y no tiene indicador... es basura. Next.
                                continue

                            # A. Buscar dentro del texto del indicador
                            # Regex mejorada: Acepta espacios o comillas al inicio
                            match = re.match(r'^\s*"?(\d+(?:\.\d+)+)', ind_content)
                            
                            if match:
                                found_id = match.group(1)
                                if self.ask_id_extraction(found_id, ctx):
                                    raw_num = found_id
...
if __name__ == "__main__":
    ...
    IPSParserHybridV105(path).process_folder()