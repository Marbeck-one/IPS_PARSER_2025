...
# =============================================================================
# IPS_HYBRID_v1.0.6 - DEEP SEARCH ID & MEMORY
# =============================================================================

class IPSParserHybridV106:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v1.0.6.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v1.0.6")
        print("="*60)
    ...
    def ask_id_extraction(self, extracted_id, context):
        ...
        print(f"   No hay columna Número, pero encontré '{extracted_id}' dentro del texto.")
    ...
    def ask_missing_id_strategy(self, prev_id, context):
        # NOTA: En v1.0.6 quitamos la memoria automática para esta decisión
        # porque en tu caso a veces hay que saltar (fila vacía) y a veces usar el anterior (fila de datos).
        # Preguntar siempre es más seguro para este archivo complejo.
        
        print(f"\n[DECISIÓN ID FALTANTE] {context}")
        print(f"   No encontré un número válido.")
        print(f"   [s]  Saltar esta fila (Probablemente basura/vacía).")
        print(f"   [p]  Usar el ID del registro anterior: '{prev_id}' (Para desgloses).")
                ...
                for loop_idx, h_idx in enumerate(header_indices):
                    ...
                    for i in range(h_idx + 1, end_idx):
                        ...
                        if not raw_num or raw_num.lower() == "nan":
                            ...
                            # A. BUSQUEDA PROFUNDA: Regex mejorada (Deep Search)
                            # Busca patrón numérico X.X.X en CUALQUIER PARTE del texto
                            match = re.search(r'(?:^|[\s\n])(\d+\.\d+\.\d+(?:\.\d+)*)', ind_content)
...
if __name__ == "__main__":
    ...
    IPSParserHybridV106(path).process_folder()