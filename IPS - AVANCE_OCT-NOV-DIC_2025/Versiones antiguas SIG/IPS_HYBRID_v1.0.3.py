...
# =============================================================================
# IPS_HYBRID_v1.0.3 - INTERACTIVE CONTROL (ID & SEGMENTS)
# =============================================================================

class IPSParserHybridV103:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v1.0.3.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
        ...
        # Memorias de decisiones
        self.decisions = {
            "use_segment": None, 
            "use_col_a_as_num": None,
            "use_embedded_id": None, # Decisión global sobre extraer ID del texto
            "missing_id_strategy": None # Decisión global sobre qué hacer si falta ID (Prev/New)
        }
        self.file_teams = {} 

    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v1.0.3")
        print("="*60)
    ...
    def ask_user_decision(self, key, prompt_text):
        # Si ya hay decisión global guardada (para segmentos específicos NO se guarda global)
        if "segment" not in key and self.decisions.get(key) is not None: 
        ...
        while True:
            ...
            if resp in ["S", "N"]:
                decision = (resp == "S")
                # Solo guardar si no es una pregunta de segmento (que es fila a fila)
                if "segment" not in key: self.decisions[key] = decision
                return decision

    def ask_id_extraction(self, extracted_id, row_idx):
        """Pregunta si usar el ID encontrado dentro del texto del indicador."""
        if self.decisions.get("use_embedded_id") is not None:
            return self.decisions["use_embedded_id"]

        print(f"\n[DECISIÓN DE ID] Fila {row_idx}: No hay columna Número, pero encontré '{extracted_id}' en el texto.")
        print("   [s]  Usar este código.")
        print("   [a]  Usar SIEMPRE códigos encontrados en el texto (Automático).")
        print("   [n]  No usar (Ir a estrategia de ID faltante).")
    ...
    def ask_missing_id_strategy(self, row_idx, prev_id):
        """Pregunta qué hacer cuando no hay ID por ninguna parte."""
        if self.decisions.get("missing_id_strategy") is not None:
            return self.decisions["missing_id_strategy"]

        print(f"\n[DECISIÓN ID FALTANTE] Fila {row_idx}: No encontré número.")
        print(f"   [p]  Usar el ID del registro anterior: '{prev_id}' (Para desgloses H/M).")
        print(f"   [pa] Usar SIEMPRE el anterior si falta.")
        print(f"   [n]  Generar código NUEVO único (GEN_X).")
        print(f"   [na] Generar SIEMPRE código nuevo.")

        while True:
            choice = input("   >> Elija: ").lower().strip()
            if choice == 'p': return 'prev'
            if choice == 'pa': 
                self.decisions["missing_id_strategy"] = 'prev'
                return 'prev'
            if choice == 'n': return 'new'
            if choice == 'na':
                self.decisions["missing_id_strategy"] = 'new'
                return 'new'

    def ask_hidden_interactive(self, count, sheet_name):
        print(f"\n[DECISIÓN] {count} filas OCULTAS en '{sheet_name}'.")
        print("   [v] Visibles [t] Todas [s] Saltar hoja")
        while True:
            choice = input("   >> Elija: ").lower().strip()
    ...
    def ask_column_action(self, missing_cols, sheet_name):
        print(f"\n[ALERTA] Hoja '{sheet_name}': Falta columna {missing_cols}")
        print("   [c] Continuar (Se intentará inferir o preguntar)")
        print("   [s] Saltar hoja")
        print("   [d] Detener")
        while True:
            choice = input("   >> Elija: ").lower().strip()
    ...
    def ask_weird_row_action(self, row_idx, content, sheet_name):
        ...
        print(f"\n[FILA RARA #{row_idx} en '{sheet_name}'] NÚMERO: '{content}'")
    ...
    def find_operand_offsets(self, df, start_row, ignored_rows):
        ...
        for offset in range(1, 12):
            ...
            if "OPERANDO 1=" in row_str or "OPERANDO 1 =" in row_str: off1 = offset + 1
            if "OPERANDO 2=" in row_str or "OPERANDO 2 =" in row_str: off2 = offset + 1
        ...
        if not off1 and not off2: # Fallback Legacy
            ...
            for offset in range(1, 10):
                ...
                if any(c.isdigit() for c in row_s) and "VALOR" not in row_s: offsets.append(offset)
    ...
    def process_folder(self):
        ...
        for idx_file, file_path in enumerate(valid_files):
            ...
            for sheet in xls.sheet_names:
                ...
                sheet_rows = []
                last_valid_id = "N/A" # Memoria para el ID anterior en el mismo sheet
                ...
                for loop_idx, h_idx in enumerate(header_indices):
                    ...
                    if h_idx > 0:
                        ...
                        if len(prev) == 1:
                            ...
                            if len(cand) < 60:
                                msg = f"Hoja '{sheet}': Título '{cand}'. ¿Es SEGMENTO?"
                    ...
                    if col_num is None:
                        action = self.ask_column_action("NÚMERO", sheet)
                    ...
                    for i in range(h_idx + 1, end_idx):
                        # --- 1. DETECCION SEGMENTO (Antes de Número) ---
                        if c_map["num"] is None or c_map["num"] == 0:
                            ...
                            if len(possible_seg) > 2 and len(possible_seg) < 30 and not any(c.isdigit() for c in possible_seg) and "INDICADOR" not in possible_seg.upper() and possible_seg != "" and possible_seg.upper() != "NAN":
                                msg = f"Fila {i}: Texto '{possible_seg}'. ¿Es SEGMENTO?"
                                if self.ask_user_decision("use_segment_inline", msg):
                                    current_segment = possible_seg
                                    continue

                        raw_num = str(df.iloc[i, c_map["num"]]).strip() if c_map["num"] is not None else ""
                        
                        # --- 2. EXTRACCION INTELIGENTE DE NUMERO ---
                        # Si no hay número en la columna oficial, buscamos alternativas
                        if not raw_num or raw_num.lower() == "nan":
                            # A. Buscar dentro del texto del indicador
                            ind_text = str(df.iloc[i, c_map["ind"]]).strip() if c_map["ind"] is not None else ""
                            match = re.match(r'^(\d+(?:\.\d+)+)', ind_text)
                            
                            if match:
                                found_id = match.group(1)
                                if self.ask_id_extraction(found_id, i):
                                    raw_num = found_id
                            
                            # B. Si sigue sin número, preguntar estrategia
                            if not raw_num or raw_num.lower() == "nan":
                                # Solo preguntar si parece una fila válida (tiene indicador o formula)
                                is_valid_row = (c_map["ind"] is not None and len(str(df.iloc[i, c_map["ind"]])) > 5)
                                if is_valid_row:
                                    strat = self.ask_missing_id_strategy(i, last_valid_id)
                                    if strat == 'prev': raw_num = last_valid_id
                                    else: 
                                        raw_num = f"GEN_{self.new_indicator_count}"
                                        self.new_indicator_count += 1
                        ...
                        if not (re.match(r'^\d', raw_num) or "NUEVO" in raw_num.upper() or "GEN" in raw_num.upper()):
                            action = self.ask_weird_row_action(i+1, raw_num, sheet)
                            if action == 'skip': continue
                            if action == 'auto': 
                                raw_num = f"GEN_{self.new_indicator_count}"
                                self.new_indicator_count += 1
                        ...
                        off1, off2 = self.find_operand_offsets(df, i, set()) # Ignored rows empty
                        ...
                        row_data = {
                            ...
                            "Meta 2026": self.transform_percentage(gd(idx_ind_data, c_map["meta"])),
                        ...
                        for m_num, m_txt in meses_v:
                            ...
                            self.variable_data.append({
                                ...
                                "VALOR_TOTAL": v1_raw, "ARCHIVO": file_name, "HOJA": sheet
                            })
                            ...
                            self.variable_data.append({
                                ...
                                "CENTRO_RESP_COD": global_center, "COD_REGION": 0,
                                "VALOR_M": "", "VALOR_F": "", "VALOR_S": "", "VALOR_J": "",
                                "VALOR_TOTAL": v2_raw, "ARCHIVO": file_name, "HOJA": sheet
                            })
    ...
    def get_ordered_headers(self):
        ...
        for m in months_order:
            base.append(f"{m} Op 1")
            base.append(f"{m} Op 2")
            
        base.append("Dic. Op 1") # Dic no tiene Acum en tu lista
        base.append("Dic. Op 2")
...
if __name__ == "__main__":
    ...
    IPSParserHybridV103(path).process_folder()