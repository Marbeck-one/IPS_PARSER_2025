...
# =============================================================================
# IPS_PARSER_v5.0.5 - PRECISION SURGEON (ETIQUETA + 1)
# =============================================================================

class IPSParserV505:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_v5.0.5.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v5.0.5")
        print("="*60)
            ...
            for r_idx, c_idx in targets:
                if r_idx < len(df) and c_idx < len(df.columns):
                    ...
                    if (len(val) > 3 and val_up != "NAN" and "INDICADOR" not in val_up and "FECHA" not in val_up and "ELABORADO" not in val_up and "NUMERO" not in val_up):
                        candidate = val
                        print(f"   [AUTO] Equipo en ({r_idx+1},{get_column_letter(c_idx+1)}): {candidate}")
                        break
    ...
    def get_real_data_row_index(self, df, start_idx, col_check_idx):
        """Si dice 'Valor indicador=', la data real está abajo (+1)."""
    ...
    def find_operand_offsets_precise(self, df, start_row, ignored_rows):
        """
        LÓGICA CORREGIDA (v5.0.5):
        Busca explícitamente la etiqueta 'Operando 1=' y devuelve (offset_etiqueta + 1).
        Esto garantiza que agarremos el VALOR (F19) y no la ETIQUETA (F18) ni el INDICADOR (F17).
        """
        offset_op1 = None
        offset_op2 = None
        max_look = 12
        ...
        for offset in range(1, max_look + 1):
            ...
            # Obtener texto de toda la fila para buscar la etiqueta
            row_str = "".join([str(x).upper() for x in df.iloc[target].values if pd.notna(x)])
            
            # REGLA DE ORO: Si encuentro la etiqueta, el dato está en la fila SIGUIENTE
            if "OPERANDO 1=" in row_str or "OPERANDO 1 =" in row_str:
                offset_op1 = offset + 1 # <--- EL DATO ESTÁ ABAJO
            
            if "OPERANDO 2=" in row_str or "OPERANDO 2 =" in row_str:
                offset_op2 = offset + 1 # <--- EL DATO ESTÁ ABAJO
                
            if offset_op1 and offset_op2: break
        
        # Fallback: Si no encuentro etiquetas (formato antiguo), uso la lógica vieja (buscar números)
    ...
    def find_operand_offsets_legacy(self, df, start_row, ignored_rows):
        """Fallback para planillas viejas sin etiquetas 'Operando X=' explícitas"""
        offsets = []
        for offset in range(1, 10):
            ...
            # Si tiene texto en columna B o C y números, asumimos operando
            if any(c.isdigit() for c in row_str) and "VALOR" not in row_str:
            ...
            for sheet in xls.sheet_names:
                ...
                for loop_idx, h_idx in enumerate(header_indices):
                    ...
                    for i in range(h_idx + 1, end_idx):
                        ...
                        # 1. Dato del Indicador (F17 si F16 es etiqueta)
                        ...
                        # 2. Dato de Operandos (F19 y F21 basado en etiquetas F18 y F20)
                        ...
                        for m_num, m_txt in meses_v:
                            ...
                            if col_m is not None:
                                v1 = gd(idx_op1, col_m)
                                if v1 and "Valor" not in str(v1) and "Operando" not in str(v1):
                                ...
                                v2 = gd(idx_op2, col_m)
                                if v2 and "Valor" not in str(v2) and "Operando" not in str(v2):
                                    self.variable_data.append({
...
if __name__ == "__main__":
    ...
    IPSParserV505(path).process_folder()