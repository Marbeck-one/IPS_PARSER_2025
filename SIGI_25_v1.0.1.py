...
=============================================================================
 ...
 DESCRIPCIÓN:
 Motor ETL (Extract, Transform, Load) diseñado para procesar planillas de 
 planificación 2025 (CDC, PMG, Riesgos) y generar los archivos maestros 
 de carga para el sistema IPS 2026.
 
 CAMBIOS v1.0.1:
 - Corrección de error de variable 'partes' vs 'parts' en Fase 3.
=============================================================================
"""
...
# =============================================================================
# 1. CONFIGURACIÓN DEL ENTORNO (ÁREA DE AJUSTES)
# =============================================================================

# [IMPORTANTE] CAMBIA ESTE NOMBRE SEGÚN EL ÁREA QUE ESTÉS PROCESANDO
NOMBRE_DIVISION = "División Planificación" 

# Configuración de Archivos (Entradas y Salidas)
ARCHIVOS = {
    # Entrada: El Excel que te entregan (Debe estar en la misma carpeta)
...
# Mapa Interno de Códigos Ponderados (Fase 5)
...
# =============================================================================
# 2. LIBRERÍA DE FUNCIONES (EL MOTOR)
# =============================================================================
...
# =============================================================================
# BLOQUE 3: PROCESAMIENTO POR FASES
# =============================================================================
    ...
    for idx in indices_inicio:
        ...
        for mes in meses_std:
            ...
            if col_idx:
                ...
                fila[f"{mes} Ind (%)"] = "No aplica"
                fila[f"{mes} Op1"] = "No aplica"
                fila[f"{mes} Op2"] = "No aplica"

        if "Cump. Proy." in mapa_meses_indices:
            idx_p = mapa_meses_indices["Cump. Proy."]
            fila["Cump. Proy. Ind (%)"] = df_raw.iloc[idx+1, idx_p]
            fila["Cump. Proy. Op1"] = df_raw.iloc[idx+3, idx_p]
            fila["Cump. Proy. Op2"] = df_raw.iloc[idx+5, idx_p]
...
def rearmar_diccionario_desde_consolidado(df_consolidado):
    dataframes = {}
    current_etiqueta = None
    buffer_filas = []
    columns = df_consolidado.columns
    for _, row in df_consolidado.iterrows():
        val_num = str(row['NÚMERO'])
        if val_num.startswith("---"):
            if current_etiqueta and buffer_filas:
                dataframes[current_etiqueta] = pd.DataFrame(buffer_filas, columns=columns)
            current_etiqueta = val_num.replace("---", "").strip()
            buffer_filas = []
        else:
...
def ejecutar_fase_1():
    print(f"\n[SIGI 25] EJECUTANDO FASE 1: EXTRACCIÓN ({NOMBRE_DIVISION})")
    if not os.path.exists(ARCHIVOS["MAESTRO"]):
        print(f"[ERROR] Falta archivo maestro: {ARCHIVOS['MAESTRO']}")
        return None
    ...
    for hoja_excel, etiqueta in HOJAS_CONFIG.items():
        ...
        if df is not None and not df.empty:
            fila_sep = {col: None for col in df.columns}
            fila_sep['NÚMERO'] = f"--- {etiqueta} ---"
            dfs_consolidados.append(pd.DataFrame([fila_sep]))
            dfs_consolidados.append(df)

    if not dfs_consolidados:
        print("[ERROR] No se extrajo información.")
        return None

    df_consolidado = pd.concat(dfs_consolidados, ignore_index=True)
    
    print(f"   -> Generando: {ARCHIVOS['GESTION']}")
    with pd.ExcelWriter(ARCHIVOS["GESTION"], engine='openpyxl') as writer:
        df_consolidado.to_excel(writer, sheet_name="DATOS_BRUTOS", index=False)
        df_consolidado.to_excel(writer, sheet_name="DATOS_ESTILIZADOS", index=False)
    
    aplicar_estilos_planillas(ARCHIVOS["GESTION"], "DATOS_ESTILIZADOS")
    
    return rearmar_diccionario_desde_consolidado(df_consolidado)
...
def ejecutar_fase_2(dataframes_input=None):
    print("\n[SIGI 25] EJECUTANDO FASE 2: VARIABLES IPS")
    if dataframes_input is None:
        dataframes_input = cargar_datos_consolidado()
    ...
    if dfs_consolidados:
        ...
        print(f"   -> [OK] Creado: {ARCHIVOS['VARS_IPS']}")
        return df_final
    return None

# --- FASE 3: VARIABLES APLICADAS ---
...
def ejecutar_fase_3(df_vars_input=None):
    print("\n[SIGI 25] EJECUTANDO FASE 3: VARIABLES APLICADAS")
    ...
    print(f"   -> [OK] Creado: {ARCHIVOS['VARS_APP']}")
...
def ejecutar_fase_4(dataframes_input=None):
    print("\n[SIGI 25] EJECUTANDO FASE 4: INDICADORES IPS")
    if dataframes_input is None:
        dataframes_input = cargar_datos_consolidado()
    ...
    if dfs_ind_consolidados:
        ...
        print(f"   -> [OK] Creado: {ARCHIVOS['INDS_IPS']}")

# --- FASE 5: INDICADORES APLICADOS ---
def transformar_a_indicadores_aplicados(df_origen, etiqueta_origen, contador_nuevos_global):
    ...
    for index, row in df_origen.iterrows():
        ...
        # BUSQUEDA: Devuelve "?" si no encuentra
        cod_ponderado = MAPA_PONDERADOS_INTERNO.get(responsable_clave, "?")
        
        if cod_ponderado == "?": cod_var_auto = "?"
        else: cod_var_auto = f"A_{cod_ponderado}"

        fila = {
            'INDICADOR_COD': codigo_indicador, 'NOMBRE_INDICADOR': nombre_limpio, 'JER_TIPO_COD': 1,
            'CENTRO_RESP_COD': None, 'COD_REGION': None, 'EMAIL_RESPONSABLE': 'prueba@arbol-logika.com',
            'COD_GENERO': None, 'ANO_MES_INI': 202501, 'ANO_MES_FIN': 202512, 'COD_ANALISIS_CAUSA': 'RESP_INDICADOR',
            ...
            'OBJ_ESTRATEGICO_COD': None, 'OBJ_ESPECIFICO_COD': None, 'PROD_ESTRATEGICO_COD': None,
            'COD_PROGRAMA': None, 'COD_COMPONENTE_PROG': None, 'TIPO_META_ANUAL': 'PERIODO_ANUAL',
            ...
            'CONST_A': None, 'META_202512': meta_val, 'Ponderacion': ponderacion_val, 'COD_PONDERADO': cod_ponderado,
...
def ejecutar_fase_5(dataframes_input=None):
    print("\n[SIGI 25] EJECUTANDO FASE 5: INDICADORES APLICADOS")
    if dataframes_input is None:
        dataframes_input = cargar_datos_consolidado()
        if not dataframes_input: return
    ...
    if dfs_consolidados:
        ...
        print(f"   -> [OK] Creado: {ARCHIVOS['INDS_APP']}")
    else:
        print("   -> [AVISO] No se generaron datos.")

# =============================================================================
# BLOQUE 6: UTILS CARGA CONSOLIDADA
# =============================================================================

def cargar_datos_consolidado():
    if not os.path.exists(ARCHIVOS["GESTION"]):
        print(f"[ERROR] Falta {ARCHIVOS['GESTION']}. Ejecuta Fase 1."); return None
    print(f"   -> Cargando: {ARCHIVOS['GESTION']}...")
    try:
        df_consolidado = pd.read_excel(ARCHIVOS["GESTION"], sheet_name="DATOS_BRUTOS")
        return rearmar_diccionario_desde_consolidado(df_consolidado)
    except Exception as e: print(f"[ERROR] Carga fallida: {e}"); return None

def aplicar_estilos_planillas(n, h): 
    try:
...
# =============================================================================
# BLOQUE 7: MENÚ PRINCIPAL
# =============================================================================

def menu_principal():
    while True:
        print("\n" + "="*60)
        print("   PROGRAMA: SIGI 25")
        print(f"   DIVISIÓN: {NOMBRE_DIVISION}")
        print("="*60)
        print("   1. Ejecutar TODO (Flujo Completo)")
        print("   2. Fase 1: Extracción (Genera Planilla SIG)")
