...
import sys
import glob
import re # IMPORTANTE: Librería para procesar texto
from openpyxl import load_workbook, Workbook
from openpyxl.styles import PatternFill, Border, Side, Alignment, Font
from openpyxl.utils import get_column_letter

# =============================================================================
# IPS_PARSER_v3.9.0 - PARSING DE DIMENSIÓN Y ÁMBITO
# =============================================================================

class IPSParserV390:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V3.9.0.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v3.9.0")
        print("="*60)
    ...
    # --- NUEVA FUNCIONALIDAD: PARSING DE INDICADOR ---
    def parse_indicator_text(self, text):
        """
        Analiza el texto del indicador para extraer Dimensión y Ámbito.
        Formato esperado linea 1: "1) Calidad/Producto" o "Eficacia/Proceso"
        """
        ...
        # Regex: 
        # 1. ^[\d\)\.\-\s]* -> Ignora números iniciales "1)", "1.", etc.
        # 2. ([^/]+)         -> Captura todo hasta el slash (Dimensión)
        # 3. /               -> El separador obligatorio
        # 4. (.+)            -> Captura el resto de la línea (Ámbito)
        ...
        if match:
            ...
            # Reconstruir el texto del indicador sin la primera línea
            if len(lines) > 1:
                clean_text = "\n".join(lines[1:]).strip()
            else:
                clean_text = "" # Si solo había título, el indicador queda vacío
                
            return clean_text, dim, amb
        else:
            # Si no cumple formato "Algo/Algo", devolvemos todo como indicador
            return text_str, "No identificado", "No identificado"
            ...
            for sheet in sheet_names:
                ...
                for i in range(h_idx + 1, len(df)):
                    ...
                    row_data = {
                        ...
                        "INDICADOR": clean_ind_text, # Texto limpio
                        "DIMENSIÓN": dim_text,       # Nueva Columna
                        "ÁMBITO": amb_text,          # Nueva Columna
                    ...
                    # 2. DATOS_VARIABLE (SIN cambios en columnas)
