...
# =============================================================================
# IPS_PARSER_v3.2.0 - DATOS_VARIABLE (OCT-DIC) + LÓGICA A/B
# =============================================================================

class IPSParserV320:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V3.2.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] # NUEVO: Para almacenar la hoja DATOS_VARIABLE
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v3.2")
        print("="*60)
        ...
        for idx_file, file_path in enumerate(files):
            ...
            for sheet in sheet_names:
                ...
                for i in range(h_idx + 1, len(df)):
                    ...
                    # 1. GENERAR ROW PRINCIPAL (CARGA BRUTA)
                    row_data = {
                    ...
                    # 2. GENERAR ROWs PARA DATOS_VARIABLE (Oct, Nov, Dic)
                    # Meses de interés: Oct(10), Nov(11), Dic(12)
                    # Claves en month_map: "Oct.", "Nov.", "Dic."
                    
                    meses_vars = [(10, "Oct."), (11, "Nov."), (12, "Dic.")]
                    centro_resp = get_val(c_map["resp"])
                    
                    for mes_num, mes_key in meses_vars:
                        # Obtenemos índices de columna del mes
                        c_idx_mes = month_map[mes_key]
                        
                        # Extraemos valores usando offsets: 3 para Op1, 5 para Op2
                        val_op1 = get_val(c_idx_mes, 3)
                        val_op2 = get_val(c_idx_mes, 5)
                        
                        # Variable A (Operando 1)
                        var_a = {
                            "ANO": 2025,
                            "MES": mes_num,
                            "VARIABLE_COD": f"{final_code}_A",
                            "CENTRO_RESP_COD": centro_resp,
                            "COD_REGION": 0,
    ...
    def export_excel(self):
        ...
        # BUCLE DE SEGURIDAD PARA GUARDAR
        while True:
            try:
