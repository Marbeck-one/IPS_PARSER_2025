...
# =============================================================================
# IPS_PARSER_v4.0.1 - LIMPIEZA PROFUNDA DE PARENTESIS
# =============================================================================

class IPSParserV401:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V4.0.1.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v4.0.1")
        print("="*60)
    ...
    def parse_indicator_text(self, text):
        ...
        # Bucle para quitar paréntesis envolventes repetidos: ((Texto)) -> Texto
        while clean_text.startswith("(") and clean_text.endswith(")"):
            clean_text = clean_text[1:-1].strip()

        return clean_text, dim, amb

    # --- LÓGICA 3: ANÁLISIS DE FÓRMULA (UPDATED v4.0.1) ---
    def analyze_formula(self, formula_raw):
        ...
        # 2. Bucle agresivo para quitar TODOS los paréntesis envolventes
        # Si entra como "((A/B)*100)", el bucle lo limpia.
        # Si entra como "(A/B)*100", NO entra al bucle (porque termina en 0, no en ")")
        while f_clean.startswith("(") and f_clean.endswith(")"):
             f_clean = f_clean[1:-1].strip()
        
        # 3. Determinar Tipo (Busca *100 al final)
        ...
        for idx_file, file_path in enumerate(files):
            ...
            for sheet in sheet_names:
                ...
                for i in range(h_idx + 1, len(df)):
                    ...
                    # 1. PARSING DE INDICADOR (Con limpieza extra de parentesis)
                    raw_ind_text = get_val(c_map["ind"])
                    clean_ind_text, dim_text, amb_text = self.parse_indicator_text(raw_ind_text)

                    # 2. PARSING DE FÓRMULA (Con limpieza extra de parentesis)
