...
# =============================================================================
# IPS_PARSER_v2.1.0 - VISIÓN HUMANA (Ignora Filas Ocultas)
# =============================================================================

class IPSParserV210:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V2.1.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
    ...
    def get_hidden_rows(self, file_path, sheet_name):
        """Devuelve un set con los índices (0-based) de las filas ocultas."""
        try:
            ...
            for row_idx, row_dim in ws.row_dimensions.items():
                if row_dim.hidden:
                    # Excel es 1-based, Pandas es 0-based. Restamos 1.
                    hidden.add(row_idx - 1) 
            wb.close()
            return hidden
        except Exception as e:
            print(f"  [AVISO] No se pudo detectar filas ocultas: {e}")
            return set()
    ...
    def process_folder(self):
        ...
        for file_path in files:
            ...
            for sheet in sheet_names:
                # 1. Detectar Filas Ocultas antes de leer
                hidden_rows = self.get_hidden_rows(file_path, sheet)
                if hidden_rows:
                    print(f"  [VISIÓN HUMANA] Se detectaron {len(hidden_rows)} filas ocultas en '{sheet}'. Se ignorarán.")

                try:
                    df = pd.read_excel(file_path, sheet_name=sheet, header=None)
                except: continue

                # BUSCAR ENCABEZADO "NÚMERO" (Ignorando filas ocultas)
                h_idx = None
                for idx, row in df.iterrows():
                    if idx in hidden_rows: continue # Saltar si está oculta
                ...
                if h_idx is None:
                    # Si no hay encabezado visible, preguntar
                    print(f"\n>>> Hoja '{sheet}'")
                    action = self.ask_user_action("No se encontró encabezado 'NÚMERO' (o está oculto).")
                ...
                for i in range(h_idx + 1, len(df)):
                    # FILTRO MAESTRO: Si la fila está oculta en Excel, LA IGNORAMOS
                    if i in hidden_rows:
                        continue

                    def get_val(col_idx, row_offset=0):
                        target_row = i + row_offset
                        # Si la fila destino (ej: operando 2) está oculta o fuera de rango, retornamos "No aplica"
                    ...
                    # Lógica de detección (Solo filas visibles)
                    ...
                    row_data = {
                        ...
                        "Cump Proy Op 1": get_val(c_map["proy"], 3),
                        "Cump Proy Op 2": get_val(c_map["proy"], 5),
                        "% Cump Meta": self.transform_percentage(get_val(c_map["cump_meta"], 3), "% Cump Meta"),
                        "Medios Verif": get_val(c_map["medios"], 0),
                        "Control Cambios": get_val(c_map["control"], 0),
                        "Instrumentos": get_val(c_map["inst"], 0)
                    }
...
if __name__ == "__main__":
    try:
        ...
        if os.path.isdir(path):
            IPSParserV210(path).process_folder()
            # IPSParserV210(path).export_excel() # Se llama dentro o separadamente si prefieres
            # Nota: process_folder no llama a export_excel automáticamente en mi código anterior, 
            # así que instanciamos y llamamos:
            parser = IPSParserV210(path)
