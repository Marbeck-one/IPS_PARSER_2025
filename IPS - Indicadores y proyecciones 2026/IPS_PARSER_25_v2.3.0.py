...
# =============================================================================
# IPS_PARSER_v2.3.0 - FIX: DOBLE ESPACIO Y ORDEN FINAL
# =============================================================================

class IPSParserV230:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V2.3.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
    ...
    def process_folder(self):
        ...
        for file_path in files:
            ...
            for sheet in sheet_names:
                ...
                # MAPEO FLEXIBLE (Para tolerar dobles espacios)
                def find_c(names):
                    for i, h in enumerate(headers):
                        h_clean = " ".join(str(h).split()).lower() # Normaliza espacios múltiples
                        for n in names:
                ...
                c_map = {
                    ...
                    # Aquí buscamos "Operandos Estimados" normalizado (sin importar si hay 1 o 2 espacios)
                ...
                for i in range(h_idx + 1, len(df)):
                    ...
                    # Operandos (Descripciones)
                    row_data["Desc Op 1"] = get_val(c_map["op_desc"], 0)
                    row_data["Desc Op 2"] = get_val(c_map["op_desc"], 3)
                    
                    # Operandos Estimados (Metas numéricas)
                    row_data["Meta Op 1"] = get_val(c_map["op_est"], 3)
                    row_data["Meta Op 2"] = get_val(c_map["op_est"], 5)
