...
# =============================================================================
# IPS_PARSER_v1.9.0 - CONTROL TOTAL
# =============================================================================

class IPSParserV190:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_UNIVERSAL_v1.9.xlsx")
    ...
    def ask_user_action(self, context_msg):
        ...
        Permite al usuario decidir el destino del proceso.
        """
        print(f"\n[ATENCIÓN REQUERIDA] {context_msg}")
        print("  Opciones:")
        print("   [s] Saltar esta hoja y continuar con la siguiente (Asumir que no es datos).")
        print("   [d] Detener TODO el proceso inmediatamente para revisar.")
        
        while True:
            choice = input("  >> Elija una opción (s/d): ").lower().strip()
            if choice == 's':
                print("  -> Saltando hoja...")
                return 'skip'
            elif choice == 'd':
    ...
    def process_folder(self):
        ...
        for file_path in files:
            ...
            try:
                ...
                self.alert_user("Fallo crítico al abrir archivo.", critical=True)
                continue
            ...
            for sheet in sheet_names:
                ...
                if h_idx is None:
                    # AQUÍ ESTÁ EL CAMBIO SOLICITADO: NO SALTAR AUTOMÁTICAMENTE
                    action = self.ask_user_action(f"No se encontró la columna 'NÚMERO' en la hoja '{sheet}'.")
                    if action == 'skip':
                        continue
                    # Si el usuario eligiera otra lógica futura, iría aquí.
                ...
                if missing:
                    ...
                    # Aquí también podríamos preguntar, pero suele ser mejor solo avisar.
                    # Si quieres detenerte aquí también, avísame.
                ...
                for i in range(h_idx + 1, len(df)):
                    ...
                    if str_num == "" or str_num.lower() == "nan" or "NUEVO" in str_num.upper():
                        ...
                        # Validación mínima: que tenga indicador y no sea cero
...
if __name__ == "__main__":
    try:
        print("--- IPS PARSER v1.9.0 (Batch + Interactive) ---")
        path = input("Ruta de la carpeta (Enter para actual): ").strip()
        if not path: path = os.getcwd()
        
        if os.path.isdir(path):
            parser = IPSParserV190(path)
