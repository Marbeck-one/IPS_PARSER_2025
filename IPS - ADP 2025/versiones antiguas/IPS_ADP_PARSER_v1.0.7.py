...
# =============================================================================
# IPS_ADP_PARSER_v1.0.7 - NUEVA ESTRUCTURA DE COLUMNAS (Variables Desactivada)
# =============================================================================

class IPS_ADP_Parser:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "CONSOLIDADO_ADP_v1.0.7.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
        ...
        self.valid_sheet_keywords = ["PROYECCI", "SIG", "proyecci", "plantilla sig","sig", "Proyección", "Proyecciones", "Plantilla SIG"]   
        ...
        # Lista maestra que fuerza el orden estricto de las columnas
        self.ordered_keys = [
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA ADP v1.0.7")
        print("="*60)
    ...
    def process_folder(self):
        ...
        for idx_file, file_path in enumerate(files):
            ...
            self.data_tree[file_name_correct] = {}
            print(f"\n>>> PROCESANDO ({idx_file + 1}/{len(files)}): {file_name_correct}")
            
            # --- 1. Extraer nombre de Equipo desde el archivo ---
            equipo_name = "No definido"
            if "ADP" in file_name_correct.upper():
                # Cortar después de "ADP"
                parts = re.split(r'(?i)ADP', file_name_correct, maxsplit=1)
                if len(parts) > 1:
                    suffix = parts[1]
                    suffix = re.sub(r'(?i)\.xlsx?', '', suffix) # quitar extension
                    suffix = re.sub(r'\d+', '', suffix) # quitar numeros (años, meses)
            ...
            for sheet in sheet_names:
                ...
                # --- 2. Extraer Responsable (Filas 5 a 12, buscando coma) ---
                ...
                while i < len(df):
                    ...
                    if str_num != "" and str_num.upper() not in ["NAN", "NONE"]:
                        ...
                        # --- CARGA DEL DICCIONARIO CON LA ESTRUCTURA EXACTA ---
                        temp_data = {
                        ...
                        for mes in self.meses_fijos:
                            if mes in month_cols:
                                ...
                                if str(val_op1).strip() != "":
                                    self.variable_data.append({"PERIODO (Mes)": mes, "VARIABLE_COD": f"{str_num}_A", "VALOR_TOTAL": val_op1, "ARCHIVO": file_name_correct, "HOJA": sheet})
                                if str(val_op2).strip() != "":
                                    self.variable_data.append({"PERIODO (Mes)": mes, "VARIABLE_COD": f"{str_num}_B", "VALOR_TOTAL": val_op2, "ARCHIVO": file_name_correct, "HOJA": sheet})
                            else:
                                val_op1, val_op2 = "", ""
                ...
                self.data_tree[file_name_correct][sheet] = sheet_rows
    ...
    def export_excel(self):
        ...
        ws_bruta.append(self.ordered_keys)
        for r in self.flat_data: 
            ws_bruta.append([r.get(k, "") for k in self.ordered_keys])
        ...
        row_idx = 1
        FULL_WIDTH = len(self.ordered_keys)
        ...
        for fname, sheets in self.data_tree.items():
            ...
            for sname, rows in sheets.items():
                ...
                # Encabezados
                for c_i, k in enumerate(self.ordered_keys, 1):
        ...
        # 3. DATOS VARIABLES ADP (DESACTIVADA MEDIANTE "if False")
        if False and self.variable_data:
...
if __name__ == "__main__":
    try:
        print("INICIANDO PROCESADOR MASIVO ADP v1.0.7")
