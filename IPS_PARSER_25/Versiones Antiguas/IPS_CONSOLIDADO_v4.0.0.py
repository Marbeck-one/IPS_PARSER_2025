...
# =============================================================================
# IPS_PARSER_v4.0.0 - CLASIFICACIÓN DE FÓRMULAS (PORCENTAJE/CUOCIENTE)
# =============================================================================

class IPSParserV400:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V4.0.0.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v4.0.0")
        print("="*60)
    ...
    def parse_indicator_text(self, text):
        ...
        # Limpieza adicional de paréntesis envolventes en el indicador (Requerimiento 4.0)
        # Si el texto es "(Algo)" -> "Algo"
        if clean_text.startswith("(") and clean_text.endswith(")"):
            clean_text = clean_text[1:-1].strip()

        return clean_text, dim, amb

    # --- LÓGICA 3: ANÁLISIS DE FÓRMULA (NUEVO 4.0) ---
    def analyze_formula(self, formula_raw):
        if pd.isna(formula_raw) or str(formula_raw).strip() == "":
            return "", "Sin Fórmula"
        
        # 1. Limpieza base: quitar espacios y saltos de línea
        f_clean = str(formula_raw).replace("\n", " ").strip()
        
        # 2. Quitar paréntesis externos si envuelven toda la fórmula
        # Ej: (A/B) -> A/B
        # Ej: (A/B)*100 -> A/B*100 (Esto facilita la detección del *100)
        
        # Chequeo simple de balance de parentesis al inicio y fin
        # Si empieza con ( y termina con )
        if f_clean.startswith("(") and f_clean.endswith(")"):
             f_clean = f_clean[1:-1].strip()
        
        # 3. Determinar Tipo
        # Busca *100 al final, permitiendo espacios: * 100, *100
        # Regex: \* (asterisco) \s* (espacios opcionales) 100 $ (fin de linea)
        if re.search(r'\*\s*100\s*$', f_clean):
            f_type = "PORCENTAJE"
        else:
            f_type = "CUOCIENTE"
            
        return f_clean, f_type
        ...
        for idx_file, file_path in enumerate(files):
            ...
            for sheet in sheet_names:
                ...
                for i in range(h_idx + 1, len(df)):
                    ...
                    # 3. CARGA BRUTA (CON NUEVAS COLUMNAS)
                    row_data = {
                        "ARCHIVO": file_name, 
                        "HOJA": sheet, 
                        "EQUIPO": centro_uso, 
                        ...
                        "DIMENSIÓN": dim_text,
                        "ÁMBITO": amb_text,
                        "FORMULA": clean_formula,   # Texto Limpio
                        "TIPO FORMULA": type_formula, # Nueva Columna
