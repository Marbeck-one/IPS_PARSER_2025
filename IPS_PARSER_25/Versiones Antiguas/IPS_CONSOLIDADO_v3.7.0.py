...
# =============================================================================
# IPS_PARSER_v3.7.0 - CORRECCIÓN DE FALSOS POSITIVOS Y ALERTA MANUAL
# =============================================================================

class IPSParserV370:
    def __init__(self, folder_path):
        self.folder_path = folder_path
        self.output_file = os.path.join(folder_path, "IPS_CONSOLIDADO_V3.7.xlsx")
        self.data_tree = {} 
        self.flat_data = [] 
        self.variable_data = [] 
    ...
    def configure(self):
        print("\n" + "="*60)
        print("   CONFIGURACIÓN MAESTRA v3.7")
        print("="*60)
    ...
    def ask_center_resp_manual(self, sheet_name, file_name, last_found):
        ...
        print(f"\n[ALERTA DE EQUIPO] No encontré el nombre de la División/Región en '{sheet_name}'.")
        print(f"  Archivo: {file_name}")
        
        inherit_msg = f" (Valor anterior: '{last_found}')" if last_found else " (No hay anterior disponible)"
        
        print("  Opciones:")
        print("   [n] Usar 'No aplica' (Recomendado si no es CDC).")
        if last_found:
            print(f"   [h] Heredar: '{last_found}'")
        print("   [m] Escribir manual.")
        print("   [s] Saltar esta HOJA.")
        
        while True:
            choice = input("  >> Elija opción: ").lower().strip()
            if choice == 'n': return "No aplica"
            if choice == 'h' and last_found: return last_found
            if choice == 'm': return input("     >> Nombre: ").strip()
            if choice == 's': return None

    def find_center_responsibility(self, df):
        # Buscar en las primeras 15 filas y 10 columnas
        ...
        for r in range(limit_row):
            for c in range(limit_col):
                ...
                # CORRECCIÓN CLAVE: Ignorar si empieza con "RESPONSABLE" (Columna de persona)
                if val_upper.startswith("RESPONSABLE"): 
                    continue

                if "CENTRO DE RESPONSABILIDAD" in val_upper:
                    parts = val.split(":")
                    if len(parts) > 1:
                        # Si hay texto despues de los dos puntos, retornarlo
                        res = parts[1].strip()
                        if res: return res
                    # Si no hay dos puntos, o está vacío después, asumimos que el nombre está en la celda de al lado (Columna + 1)
                    if c + 1 < len(df.columns):
                        next_val = str(df.iloc[r, c+1]).strip()
                        if next_val and next_val.lower() != "nan":
                            return next_val
                    return val # Si falla todo, devuelve la celda original (pero ya filtramos RESPONSABLE)
    ...
    def process_folder(self):
        ...
        for idx_file, file_path in enumerate(files):
            ...
            # MEMORIA DE HERENCIA (Se reinicia por archivo)
            ...
            for sheet in sheet_names:
                ...
                if center_resp_name:
                    ...
                    if is_data_sheet:
                        # AQUÍ SALTA LA ALERTA SIEMPRE SI NO ENCUENTRA
                        centro_uso = self.ask_center_resp_manual(sheet, file_name, last_found_center)
                        if centro_uso is None: continue # Saltar hoja
                ...
                c_map = {
                    ...
                    "uni": find_c(["UNIDAD"]),
                    "resp": find_c(["RESPONSABLE"]), # Columna de persona
                    "gest": find_c(["GESTOR"]),
