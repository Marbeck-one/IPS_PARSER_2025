...
class IPSParserV150:
    ...
    def transform_percentage(self, val, col_name):
        """Transforma decimales a enteros y notifica en consola."""
        if pd.isna(val) or val == "" or val == "No aplica": return val
        try:
            num = float(val)
            # Si es un valor decimal pequeño (0 < x <= 1), asumimos que es porcentaje
            if 0 < abs(num) <= 1:
                new_val = round(num * 100, 2)
                # AQUÍ ESTÁ EL LOG QUE PEDISTE RECUPERAR
        ...
        for file_path in self.master_files:
            ...
            for sheet in self.sheets_to_process:
                ...
                if missing:
                    ...
                    print(f"  [INFO] Todas las columnas requeridas fueron encontradas.")
                ...
                for i in range(h_idx + 1, len(df)):
                    # Helper interno para obtener valor seguro
                    ...
                    if str_num == "" or str_num.lower() == "nan" or "NUEVO" in str_num.upper():
                        ...
                        if ind_val and str(ind_val).strip() != "":
                            ...
                            print(f"    [AVISO] Fila {i+1}: Sin código detectado. Asignando auto: {final_code}")
                            self.new_indicator_count += 1
                        else:
                            continue 
                    else:
                ...
                print(f"  [FIN HOJA] Se extrajeron {count_rows} indicadores exitosamente.")
    ...
    def export_excel(self):
        ...
        # HOJA 2: ESTILIZADA
        print("  -> Creando hoja 'Planilla Estilizada' con formato...")
